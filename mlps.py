...
class SRNN(nn.Module):
    ...
    def __init__(self, n_in, n_rec, n_out, n_t, thr, tau_m, tau_o, b_o, gamma, dt, model, classif, w_init_gain, lr_layer, t_crop, visualize, visualize_light, device):    
        ...
        # self.w_in  = nn.Parameter(torch.Tensor(n_rec[0], n_in ))
        # self.w_rec_in = nn.Parameter(torch.Tensor(n_rec[0], n_rec[0]))
        # self.w_hid  = nn.Parameter(torch.Tensor(n_rec[1], n_rec[0]))
        # self.w_rec_hid = nn.Parameter(torch.Tensor(n_rec[1], n_rec[1]))
    ...
    def init_net(self, n_b, n_t, n_in, n_rec, n_out):
        ...
        self.vo.append(torch.zeros(n_t,n_b,n_out).to(self.device))
            
        #Weight gradients
        for i in range(self.n_layer):
            self.w[i].grad = torch.zeros_like(self.w[i])
            self.w_rec[i]  = torch.zeros_like(self.w_rec[i])
    ...
    def forward(self, x, yt, do_training):
        ...
        for t in range(self.n_t-1):     # Computing the network state and outputs for the whole sample duration
            ...
            print([self.z[i][t+1].shape for i in range(self.n_layer)])
