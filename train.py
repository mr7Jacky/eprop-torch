    ...
    with torch.no_grad():   # Same here, we make sure autograd is disabled
        ...
        for batch_idx, (data, label) in enumerate(tqdm(loader)):
            ...
            if args.classif:    # Do a one-hot encoding for classification
                targets = torch.zeros(label.shape, device=device).unsqueeze(-1).expand(-1, args.n_classes)
            ...
            if args.delay_targets:
                output = output[-args.delay_targets:].sum(axis=0)
                # print(output.shape)
                output = torch.nn.functional.normalize(output, dim=1)
                # print(output.max(),targets.max())
